#!/usr/bin/env bash
# Backup diario de apsis: dump logico de la BD, retencion local y copia
...
#
# Sin cifrar, a diferencia del backup de ductual: apsis contiene efemerides y
# pasadas de satelite calculadas a partir de fuentes publicas, no datos
# personales. Si algun dia guarda algo sensible, el patron a copiar es el
# `gpg --symmetric --passphrase-file` de /opt/ductual/ops/staging/backup.sh.
#
...
DB_CONTAINER="${DB_CONTAINER:-apsis-db}"
...
db_out="$DAILY/apsis-${STAMP}.sql.${EXT}"
...
find "$DAILY" -type f -name 'apsis-*' -mtime +7 -delete
find "$WEEKLY" -type f -name 'apsis-*' -mtime +28 -delete
